...
```javascript
$ export vcn_id=$(oci network vcn create -c $compartment_id --cidr-block "10.0.0.0/16" --display-name vcn-cli-test --dns-label vcnclitest --query "data.id" --raw-output)
```

```javascript
$ export subnet_id=$(oci network subnet create --vcn-id $vcn_id -c $compartment_id --cidr-block "10.0.0.0/24" --display-name subnet-cli-test --dns-label subnetclitest --query "data.id" --raw-output)
```

```javascript
$ export internet_gateway_id=$(oci network internet-gateway create -c $compartment_id --is-enabled true --vcn-id $vcn_id --display-name IG-cli-test --query "data.id" --raw-output)
```

```javascript
$ oci network route-table create -c $compartment_id --route-rules '[{"cidrBlock":"0.0.0.0/0","networkEntityId":"'$internet_gateway_id'"}]' --vcn-id $vcn_id --display-name route-table-cli-test
```
