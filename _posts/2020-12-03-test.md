...
Obetenha o OCID do seu Tenancy.
...
Para criar uma VCN é neessário informar o compartment, cidr block, nome da VCN e um DNS label é recomendável utilizar o nome da VCN sem os caractereS especiais.
...
Adicione uma private subnet

```javascript
$ export subnet_id=$(oci network subnet create --vcn-id $vcn_id --compartment-id $compartment_id --cidr-block "10.0.0.0/24" --display-name subnet-cli-test --dns-label subnetclitest --query "data.id" --raw-output)
```
